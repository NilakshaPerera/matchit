...
@section('script')
    ...
    <script>
        ...
        function notifyUser(userId){
            ...
            $.ajax({
                url: '{{ route("payment.sendduemembershipnotification") }}',
                type: 'post',
                data: "user-id=" + userId,
            ...
            <table class="table table-striped jstable">
                    ...
                    <tbody>
                        ...
                        foreach($users as $user){
                            ...
                            $dues = $userController->getMembershipDues($user->id);
                            if(!$dues){
                                continue;
                            }
                            $lastPayment = $userController->getLastPaymentDate($user->id);
                        ?>                 
                            <tr>
                                ...
                                <td>{{ $lastPayment }}</td>
                                <td>{{ $dues }}£</td>
                                <td><a onClick="notifyUser({{ $user->id }})" href="Javascript:void(0)">Send Email</a></td>
